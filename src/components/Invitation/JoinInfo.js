...
export const JoinInfo = ({inviteLink}) => {
    ...
    return <div className="invitationBody">
        <p dangerouslySetInnerHTML={{ __html: t('InviteLink.title',
                { coordinatorName, coordinatorEmail, displayName }
            )}}>
        </p>
    ...
    function renderUsers({labelKey, users}) {
        ...
        return <>
            <h3>{t(labelKey)}</h3>
            <ul>{users.map(({displayName: userName, email: userEmail}, index) => <li key={`${labelKey}-${index}`}>{userName} ({userEmail})</li>)}</ul>
        </>
    }
}
